...
const UserProfile2 = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        const response = await axios.get(`https://aronnok.vercel.app/api/user/${userId}`); // Replace with actual backend URL
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.put(`https://aronnok.vercel.app/api/user/${userId}`, userInfo); // Replace with actual backend URL
      setEditing(false);
    } catch (error) {
