...
const CartItem = (prod) => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await fetch(`https://aronnok.vercel.app/api/product/${prod.userId}/${prod.id}`);
    ...
    //   try {
    //     const response = await fetch(`https://aronnok.vercel.app/api/cart/getQuantity/${prod.userId}/${prod.id}`);
    //     const data = await response.json();
  ...
  const updateCart = async (newQuantity) => {
    try {
      await axios.put(`https://aronnok.vercel.app/api/cart/update/${prod.userId}/${prod.id}`, { "quantity": newQuantity, "selected": true });
  ...
  const removeItem = async () => {
    try {
      await axios.delete(`https://aronnok.vercel.app/api/cart/deleteItem/${prod.userId}/${prod.id}`, {});
